// Code generated by mockery v2.53.3. DO NOT EDIT.

package mocks

import (
	context "context"
	domain "earnforglance/server/domain/directory"

	mock "github.com/stretchr/testify/mock"
)

// ExchangeRateUsecase is an autogenerated mock type for the ExchangeRateUsecase type
type ExchangeRateUsecase struct {
	mock.Mock
}

// Create provides a mock function with given fields: c, exchange_rate
func (_m *ExchangeRateUsecase) Create(c context.Context, exchange_rate *domain.ExchangeRate) error {
	ret := _m.Called(c, exchange_rate)

	if len(ret) == 0 {
		panic("no return value specified for Create")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *domain.ExchangeRate) error); ok {
		r0 = rf(c, exchange_rate)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Delete provides a mock function with given fields: c, ID
func (_m *ExchangeRateUsecase) Delete(c context.Context, ID string) error {
	ret := _m.Called(c, ID)

	if len(ret) == 0 {
		panic("no return value specified for Delete")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, string) error); ok {
		r0 = rf(c, ID)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Fetch provides a mock function with given fields: c
func (_m *ExchangeRateUsecase) Fetch(c context.Context) ([]domain.ExchangeRate, error) {
	ret := _m.Called(c)

	if len(ret) == 0 {
		panic("no return value specified for Fetch")
	}

	var r0 []domain.ExchangeRate
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context) ([]domain.ExchangeRate, error)); ok {
		return rf(c)
	}
	if rf, ok := ret.Get(0).(func(context.Context) []domain.ExchangeRate); ok {
		r0 = rf(c)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]domain.ExchangeRate)
		}
	}

	if rf, ok := ret.Get(1).(func(context.Context) error); ok {
		r1 = rf(c)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FetchByID provides a mock function with given fields: c, ID
func (_m *ExchangeRateUsecase) FetchByID(c context.Context, ID string) (domain.ExchangeRate, error) {
	ret := _m.Called(c, ID)

	if len(ret) == 0 {
		panic("no return value specified for FetchByID")
	}

	var r0 domain.ExchangeRate
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, string) (domain.ExchangeRate, error)); ok {
		return rf(c, ID)
	}
	if rf, ok := ret.Get(0).(func(context.Context, string) domain.ExchangeRate); ok {
		r0 = rf(c, ID)
	} else {
		r0 = ret.Get(0).(domain.ExchangeRate)
	}

	if rf, ok := ret.Get(1).(func(context.Context, string) error); ok {
		r1 = rf(c, ID)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Update provides a mock function with given fields: c, exchange_rate
func (_m *ExchangeRateUsecase) Update(c context.Context, exchange_rate *domain.ExchangeRate) error {
	ret := _m.Called(c, exchange_rate)

	if len(ret) == 0 {
		panic("no return value specified for Update")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *domain.ExchangeRate) error); ok {
		r0 = rf(c, exchange_rate)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// NewExchangeRateUsecase creates a new instance of ExchangeRateUsecase. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewExchangeRateUsecase(t interface {
	mock.TestingT
	Cleanup(func())
}) *ExchangeRateUsecase {
	mock := &ExchangeRateUsecase{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
