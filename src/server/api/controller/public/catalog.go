...
func (cc *CatalogController) GetManufacturers(c *gin.Context) {
	var request domain.ManufacturerRequest
	...
	productResponse, err := cc.CatalogUsecase.GetManufacturers(c, request)
	if err != nil {
		c.JSON(http.StatusNotFound, common.ErrorResponse{Message: err.Error()})
		return
	}
