...
func (ur *permissionrecordRepository) Delete(c context.Context, permissionrecord *domain.PermissionRecord) error {
	collection := ur.database.Collection(ur.collection)
...
func (tr *permissionrecordRepository) FetchByID(c context.Context, permissionrecordID string) (domain.PermissionRecord, error) {
	...
	idHex, err := primitive.ObjectIDFromHex(permissionrecordID)
	if err != nil {
